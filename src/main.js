...
function renderLanding() {
  ...
  if (btnCreate) {
    btnCreate.onclick = async () => {
      ...
      const perPoolEl = document.getElementById('t-perpool');
      const qPerPoolEl = document.getElementById('t-qperpool');
      
      const admin_code = Math.random().toString(36).slice(2,8).toUpperCase();
      const newT = await updateTournament({
        name,
        max_teams: maxEl ? (parseInt(maxEl.value) || 16) : 16,
        teams_per_pool: perPoolEl ? (parseInt(perPoolEl.value) || 4) : 4,
        qualifiers_per_pool: qPerPoolEl ? (parseInt(qPerPoolEl.value) || 2) : 2,
        admin_code,
        status: 'registration'
      });
...
function viewRegistration() {
  ...
  if (state.teams.length === 0) {
    html += `<p style="font-size:13px; color:var(--text3); text-align:center;">Aucune équipe inscrite.</p>`;
  }
...
function bindEvents() {
  ...
  document.querySelectorAll('.btn-save-inline-pool').forEach(btn => {
    btn.onclick = async () => {
      ...
      const s1El = document.getElementById(`s1-${id}`);
      const s2El = document.getElementById(`s2-${id}`);
  ...
  document.querySelectorAll('.btn-save-inline-bracket').forEach(btn => {
    btn.onclick = async () => {
      const id = btn.dataset.id;
      const s1El = document.getElementById(`bs1-${id}`);
      const s2El = document.getElementById(`bs2-${id}`);
      const s1 = s1El ? parseInt(s1El.value) : NaN;
      const s2 = s2El ? parseInt(s2El.value) : NaN;
...
async function init() {
  ...
  if (!state.tournament) {
    const appEl = document.getElementById('app');
    if (appEl) appEl.innerHTML = `<div class="card" style="text-align:center;margin:20px;"><p>Tournoi introuvable.</p></div>`;
    return;
  }
